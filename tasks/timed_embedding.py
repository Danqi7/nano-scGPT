...
if __name__ == "__main__":
    ...
    # Warm up the hardware for timing.
    with torch.no_grad(), torch.amp.autocast(device_type=device.type, enabled=device.type=="cuda"):
        for idx, batch in enumerate(dataloader):
            ...
            _ = model.encode(gene_ids, exprs, padding_mask)
            if idx >= 10: # warm up for 10 batches
                break
    ...
    print(f"Total time for processing {embeddings.shape[0]} cells: {total_time:.2f} seconds, average {total_time/embeddings.shape[0]*1000:.3f} ms per cell.")
